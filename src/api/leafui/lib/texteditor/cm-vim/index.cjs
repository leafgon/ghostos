...
var history__namespace = /*#__PURE__*/_interopNamespace(history);
...
function dispatchChange(view, transaction) {
    if (transaction.annotations) {
        try {
            transaction.annotations.some(function (note) {
                if (note.value == "input")
                    note.value = "input.type.compose";
            });
        }
        catch (e) {
            console.error(e);
...
class CodeMirror {
    ...
    findMatchingBracket(pos) {
        ...
        var m = matchbrackets.matchBrackets(state, offset + 1, -1);
        if (m && m.end) {
            return { to: posFromIndex(state.doc, m.end.from) };
        }
        m = matchbrackets.matchBrackets(state, offset, 1);
        if (m && m.end) {
            return { to: posFromIndex(state.doc, m.end.from) };
        }
        return { to: undefined };
...
// --------------------------
CodeMirror.Pos = Pos;
CodeMirror.StringStream = streamParser.StringStream;
CodeMirror.commands = {
    cursorCharLeft: function (cm) { commands.cursorCharLeft(cm.cm6); },
    redo: function (cm) { history__namespace.redo(cm.cm6); },
    undo: function (cm) { history__namespace.undo(cm.cm6); },
...
function findMatchingTag(cm, pos) {
    ...
    var m = matchbrackets.matchBrackets(state, offset + 1, -1, { brackets: "\n\n" });
    if (m) {
        if (!m.end || !m.start)
            return;
        return {
    ...
    m = matchbrackets.matchBrackets(state, offset, 1, { brackets: "\n\n" });
    if (m) {
        if (!m.end || !m.start)
            return;
        return {
...
function findEnclosingTag(cm, pos) {
    ...
    var i = offset;
    while (i > 0) {
        var m = matchbrackets.matchBrackets(state, i, 1, { brackets: "\n\n" });
        if (m && m.start && m.end) {
            return {
                open: convertRange(state.doc, m.start),
                close: convertRange(state.doc, m.end),
...
const hideNativeSelection = state.Prec.override(view.EditorView.theme(themeSpec));
...
const vimPanelState = state.StateField.define({
    ...
    provide: f => {
        return panel.showPanel.from(f, on => on ? createVimPanel : null);
    }
});
...
function vim(options = {}) {
    return [
        ...
        vimPlugin,
        hideNativeSelection,
        options.status ? panel.showPanel.of(statusPanel) : vimPanelState
    ];
}
function getCM(view) {
    return view.cm || null;
