...
// also featured in https://codesandbox.io/s/projection-mapping-demo-3-or6l4?from-embed=&file=/src/index.js:758-764
import pMap from 'p-map'
import prettyMs from 'pretty-ms'
import loadImage from 'image-promise'
import { GLTFLoader } from 'three/examples/jsm/loaders/GLTFLoader.js'
import loadTexture from './loadTexture.js'
import loadCubeTexture from './loadCubeTexture.js'
...
class AssetManager {
  ...
  constructor() {
    ...
    console.log('Instantiating AssetManager, debug mode:', globalThis.DEBUG);
  }
  ...
  _extractType(url) {
    ...
    switch (true) {
      ...
      case /\.svg$/i.test(ext):
        return 'svg'
      case /\.(jpe?g|png|gif|bmp|tga|tif|hdr)$/i.test(ext):
        return 'image'
    ...
    await pMap(
      ...
      async (item, i) => {
        try {
          ...
          that.#cache[item.url] = await that._loadItem({ renderer, ...item })
          ...
          that.logError(`Asset ${item.url} was not loaded:\n${err}`)
        }

        const percent = (i + 1) / total
        that.#onProgressListeners.forEach((fn) => fn(percent))
      },
      { concurrency: that.#asyncConcurrency }
    )
  ...
  _loadItem({ url, type, renderer, ...options }) {
    ...
    switch (type) {
      ...
      case 'cubetexture':
        return loadCubeTexture(url, { renderer, ...options })
      case 'audio':
...
const assets_manager = new AssetManager();
