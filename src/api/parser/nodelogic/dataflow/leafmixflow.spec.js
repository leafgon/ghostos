...
const { zipWith, tap, connect, map, take, concatMap } = require('rxjs/operators');

const { doBottle } = require('../../../../../ghostos/api/parser/nodelogic/datautils');

const { _leafmemoryio } = require('../../../../../ghostos/api/parser/nodelogic/io');
const { _leafmixflow } = require('../../../../../ghostos/api/parser/nodelogic/dataflow');
...
describe("ghostos/api/parser function tests", () => {
  const nodes = [
    ...
    },
    {
      ...
      "out_edges":[],
      "data": encodeNodeData(
        {"leaf":{"logic":{"type":"leafmixflow","args":{mixOperator: "merge"}},"api":"breezyforest","appdata":{"position":{"x":0,"y":0}}},"domain":testdomain,"appid":testappid,"inputData":{},"isdebug":true,"selected":true}
      )
    }
  ];
  ...
  test("testing leafspell dataflow function", async () => {
    ...
    const dataflowlogic = await _leafmixflow.dataflow(lambdactrl)({refnode: "7871c31a-8564-44a0-bd0d-8b17da18653f", refnodedata: {leaf: {logic: {args: leafnode_args}}}, nodelambda: [], contextuallambda: []});

    const datainput1$ = new ReplaySubject(1); //of([doBottle('accio', 'accio')]).pipe(take(1)); // an arbitrarily chosen string to function as standard issue invocation input data
    const datainput2$ = new ReplaySubject(1); //of([doBottle('accio', 'accio')]).pipe(take(1)); // an arbitrarily chosen string to function as standard issue invocation input data
    const datainput3$ = new ReplaySubject(1); //of([doBottle('accio', 'accio')]).pipe(take(1)); // an arbitrarily chosen string to function as standard issue invocation input data
    ...
    const peepedinput$ = datainput1$.pipe(
        map(x => {
    ...
    const output$obj = dataflowlogic([{_stream: peepedinput$},{_stream: datainput2$}, {_stream: datainput3$}], controlflow$obj);

    datainput1$.next(data_accio_bottle);
    datainput2$.next(2);
    datainput3$.next(3);
    ...
    expect(outcache).toBe(JSON.stringify([1, 2, 3]));
