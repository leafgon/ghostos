...
const initElementInterface = (_stream_cache) => {
    ...
    const node_context = {
        codebase: "ghostos/api/parser/nodelogic/elements/library/html.js",
        ..._stream_cache
    };
    ...
    firstValueFrom(template$obj._control._stream.pipe(withLatestFrom(template$obj._stream))).then(_combined_in => {
        ...
        Handlebars.registerHelper('markdown', function(text) {
            ...
            const text_b64 = Buffer.from(text).toString("base64");
            
            //return `JSON.parse(Buffer.from('${context_str_b64}', 'base64').toString())`;
            return `atob('${text_b64}')`;
        });
        Handlebars.registerHelper('json', function(context) {
            ...
            // as per https://stackoverflow.com/questions/38134200/base64-encode-a-javascript-object
            const context_str_b64 = Buffer.from(JSON.stringify(context, jsonnum_replace)).toString("base64");
            
            //return `JSON.parse(Buffer.from('${context_str_b64}', 'base64').toString())`;
            return `JSON.parse(atob('${context_str_b64}'))`;
        });
    ...
    const _do_datalogic = (data_in, _template_fn) => {
        ...
        if (_template_fn !== undefined) {
            const template_keys = Object.keys(_template_fn);

            const _data_out = (template_keys.length === 1 && template_keys.includes('_default')) ? 
            [_template_fn["_default"](Object.assign({}, ...([data_in].flat().map(_datum_in=>doUnbottle("*", _datum_in)))))] : // merge all input dictionaries after unbottling them first, then apply into the template
            [data_in].flat().sort((a,b)=>{
        ...
        const _ctrl_out$ = controlflow$obj._stream.pipe(
            ...
            map(_combined_in => {
                ...
                if (isBottle(_data_in) && _data_in._bname === "elementio") {
                    ...
                    const processed_html = _do_datalogic(_data_in, _template_fn);
