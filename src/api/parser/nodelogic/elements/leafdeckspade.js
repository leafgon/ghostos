...
import { combineDataflows, naiveZipDataflows, mergeDataflows, chronosDataflow, CHRONOSTYPE_SYNC } from '../../../utils/leafdataflow.js';
import { v4 as uuid4, v5 as uuid5 } from 'uuid';
import { _leafgraph } from '../abstraction/index.js';
import { driveDataflowByCtrlflow, executeLEAFLogicInSync } from '../../leaf.js';
...
const leaf_apidef = _leafstdlib_dataflow_api("leafdeckspade");
/*
 * _leafdeckspade() is a runtime function for the LEAFgraph node of logic type 'leafdeckspade'.
 * it is used to generate a navigable node of type spade upon any diamond bottled data being passed to the leaf node 
 * via its incoming data flow. 
 * (25 Feb 2022) How the dataflow plane input gets incorporated into the leaf node's output is yet to be determined. 
 ...
 * the View of MVI.
 * 
 * Upon the leafdeckspade node logic executing, a gnav-compatible json object to represent 
 * the leafdeckspade in the rhelm of gnav.js is expected to be returned. 
 * 
 * TBD:
 * @graphContextual: n/a 
 * or
 ...
 * '_leafdeckdiamond'
 */
//const _leafdeckspade = ({logictoggle=false, deckname=''}={}, lambdaFunc=x=>x, graphContextual) => {}
const _leafdeckspade = {
    dataflow: (lambdactrl) => async ({refnode, refnodedata, nodelambda, contextuallambda}={}) => {
        ...
        //const filterDataOfInterest = await _leafunbottle.dataflow(lambdactrl)({refnodedata: {leaf: {logic: {args: {bottlekey:'_leafdeckdiamond'}}}}, nodelambda: [], contextuallambda});
        ...
        //const bottleUp = await _leafbottle.dataflow(lambdactrl)({refnodedata: {leaf: {logic: {args: {bottlekey:'_leafdeckspade'}}}}, nodelambda, contextuallambda});

        //let default_spade_diamonds_and_clubs = [
        //    {
        //        type: 'leafdeckdiamond',
        //        uuid: uuid4(),
        //        name: 'somename',
        //        svgicon: 'http://localhost:3000/icons/hardware/laptop_mac/materialicons/24px.svg',
        //        description: 'some description',
        //        lambdas: {
        //            default: undefined, //'defaultLambda',
        //            //defaultLambda: (input$Arr) => {}
        //        },
        //        clubs: [
        //        ]
        //    }
        ...
        const leafgraph_args = {graphuuid: '', domain: 'breezyforest', appid: 'leafdeckspade', graphaddrstr: 'breezyforest/spade'};
        ...
        //let default_spade_diamonds_and_clubs = []; //{diamonds: []};
        //if (leafgraph && 'menudata' in leafgraph && '_default' in leafgraph.menudata) {
        //    const menudata = await executeLEAFLogic(leafgraph.menudata._default, []);
        //    if (menudata.length > 0) {
        //        default_spade_diamonds_and_clubs = menudata[0].diamonds;
        //        console.log('the leaf standard lib for leafdeckspade has been loaded.', leafgraph);
        //    }
        //    else {
        //        console.log('the leaf standard lib for leafdeckspade has been loaded with zero default menu entries.', leafgraph);
        //    }
        //}
        //else {
        //    console.error('the leaf standard lib for leafdeckspade could not be loaded.');
        //}

        const node_context = {
            codebase: "leafdeckspade.js",
            refnode, 
        ...
        return (input$objArr, controlflow$obj) => {
            ...
            const _leafdeckspade_leaflogic = (_input$objArr, _controlflow$obj) => {
            ...
            const output$obj = driveDataflowByCtrlflow(
                ...
                {...node_context, metastep: "creating a leafdeckspade element"},
                {
                    //leaflogic: _leafdeckspade_leaflogic,
                    datalogic: {
                        pre: preprocessfunc,
