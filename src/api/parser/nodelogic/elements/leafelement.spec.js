...
const { doBottle } = require('../../../../../ghostos/api/parser/nodelogic/datautils');
...
const { _leafelement } = require('../../../../../ghostos/api/parser/nodelogic/elements');

const { ElemCacheKeyLUT } = require("./library");
...
describe("leafelement standard function tests", () => {
  const initNodes = (_elementname) => {
    return [
        {
        ...
        "out_edges":[{"uuid":"ae68998b-7fa5-4fbd-bcfe-f6b0fdd0c29c","source":{"uuid":"0f663e30-9691-4cc3-95ab-3a12a027bb8e"},"target":{"uuid":"496926a2-1ece-4fa1-9260-6e4a11f24ed1"},"data":nodedataLUT.edges.data}],
        "data": encodeNodeData(
            {"leaf":{"logic":{"type":"leafelement","args":{
                elementname: _elementname, // e.g 'mediaplayer' or 'http'
            ...
            }},"api":"breezyforest","appdata":{"position":{"x":0,"y":0}}}}
        )
        },
        {
        "uuid":"496926a2-1ece-4fa1-9260-6e4a11f24ed1",
        "out_edges":[],
        "data": encodeNodeData(
        ...
        "data": encodeNodeData(
            {"leaf":{"logic":{"type":"leaflisp","args":{"lispexpression":"inport;(+ (parse inport) 2)"}},"api":"breezyforest","appdata":{"position":{"x":288,"y":272}}}}
        )
        },
        {
  ...
  it("testing leafelement mediaplayer dataflow function", async () => {
    //jest.setTimeout(100000);
    // actual test
    const mode = "nav";
    const domain = testdomain;
    const appid = testappid;
    const nodes = initNodes("mediaplayer");
    ...
    const out$ = output$obj._control._stream.pipe(
        ...
        map( combined_input => {
            ...
            console.log("output data next:", x);
    ...
    console.log("got the synchronized result");
    expect(outval).toBe(JSON.stringify(
        {
            _bname:"screenio",
            _content: {
                element: {
                    nodeuuid: "0f663e30-9691-4cc3-95ab-3a12a027bb8e",
                    type: "mediaplayer",
                    componentdata: {
                        margin: 1,
                        fieldwidth: "25ch",
                        graphdomain: "breezyforest",
                        graphappid: "jestleafspell",
                        refnode:"0f663e30-9691-4cc3-95ab-3a12a027bb8e",
                        nodelambda:[],
                        contextuallambda:[]
                    }
                }
  ...
  it("testing leafelement prompt dataflow function", async () => {
    //jest.setTimeout(100000);
    // actual test
    const mode = "nav";
    const domain = testdomain;
    const appid = testappid;
    const nodes = initNodes("prompt");
    const leafgraph = reconstructLEAFGraph(nodes);
  ...
  it("testing leafelement popup dataflow function", async () => {
    //jest.setTimeout(100000);
    // actual test
    const mode = "nav";
    const domain = testdomain;
    const appid = testappid;
    const nodes = initNodes("popup");
    const leafgraph = reconstructLEAFGraph(nodes);
    ...
    const peepedinput$ = datainput$.pipe(
        map(x => {
            console.log('peephole:', x);
            //return x;
            return doBottle("screenio", 2); // popup element expects a screenio bottle in the input.
        }),
    );
    const controlflow$obj = {_stream: ctrlinput$.pipe(
        map(x => {
  ...
  it("testing leafelement editor dataflow function", async () => {
    //jest.setTimeout(100000);
    // actual test
    const mode = "nav";
    const domain = testdomain;
    const appid = testappid;
    const nodes = initNodes("editor");
    const leafgraph = reconstructLEAFGraph(nodes);
  ...
  it("testing leafelement text dataflow function", async () => {
    //jest.setTimeout(100000);
    // actual test
    const mode = "nav";
    const domain = testdomain;
    const appid = testappid;
    const nodes = initNodes("text");
    const leafgraph = reconstructLEAFGraph(nodes);
    ...
    const leafnode_args = {
        ...
        // need something here for text such as refnodedata (lambdarefnodedata)?
    };
    const dataflowlogic = await _leafspell.dataflow(lambdactrl)({refnode: "7871c31a-8564-44a0-bd0d-8b17da18653f", refnodedata: {leaf: {logic: {args: leafnode_args}}}, nodelambda: [], contextuallambda: []});
  ...
  it("testing leafelement gnav dataflow function", async () => {
    //jest.setTimeout(100000);
    // actual test
    const mode = "nav";
    const domain = testdomain;
    const appid = testappid;
    const nodes = initNodes("gnav");
    const leafgraph = reconstructLEAFGraph(nodes);
    ...
    const peepedinput$ = datainput$.pipe(
        map(x => {
            ...
            //return x;
            //return [];
            return doBottle("screenio", x); // gnav element expects a screenio bottle in the input.
        }),
    );
    const controlflow$obj = {_stream: ctrlinput$.pipe(
        map(x => {
  ...
  it("testing leafelement http dataflow function", async () => {
    //jest.setTimeout(100000);
    // actual test
    const mode = "nav";
    const domain = testdomain;
    const appid = testappid;
    const nodes = initNodes("http");
    const leafgraph = reconstructLEAFGraph(nodes);
