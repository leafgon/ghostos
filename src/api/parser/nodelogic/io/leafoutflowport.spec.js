...
const { skip, from, of, repeat, delay, interval, withLatestFrom, ReplaySubject, firstValueFrom, combineLatest, merge } = require('rxjs');
const { zipWith, tap, connect, map, take } = require('rxjs/operators');

const { _leafmemoryio, _leafoutflowport } = require('../../../../../ghostos/api/parser/nodelogic/io');
...
//      "data":"eyJsZWFmIjp7ImxvZ2ljIjp7InR5cGUiOiJsZWFmc3BlbGxkZWYiLCJhcmdzIjp7InNwZWxsbmFtZSI6Imxpc3RsaWIiLCJzdmdpY29uIjp7InVybCI6IiJ9fX0sImFwaSI6ImJyZWV6eWZvcmVzdCIsImFwcGRhdGEiOnsicG9zaXRpb24iOnsieCI6NDgsInkiOjYwOH19fX0="
  ...
  test("testing leafoutflowport dataflow function", async () => {
    // actual test
    const mode = "nav";
    const domain = "spark";
    ...
    const dataflowlogic = await _leafoutflowport.dataflow(lambdactrl)({refnode: "0b07305b-3720-4019-86db-77cc09b59684", refnodedata: {leaf: {logic: {args: leafnode_args}}}, nodelambda: [], contextuallambda: []});
    ...
    //combineLatest([output$obj._stream, output$obj._control._stream]).subscribe(
    //    {
    //        next: x => {
